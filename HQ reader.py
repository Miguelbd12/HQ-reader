...
def extract_invoice_data(text):
    ...
    if total_due_match:
        ...
        try:
            if "." in total_amount and "," in total_amount:
                total_amount = total_amount.replace(".", "").replace(",", ".")
            elif "," in total_amount and "." in total_amount:
                total_amount = total_amount.replace(",", "")
            elif "," in total_amount:
                total_amount = total_amount.replace(",", ".")
    ...
    if customer_match:
        ...
        customer = re.sub(r"GTIMA", "", customer, flags=re.IGNORECASE)
...
if uploaded_files:
    ...
    for uploaded_file in uploaded_files:
        ...
        try:
            ...
            if len(images) > 1:
                if st.checkbox(f"Show and OCR all pages in {uploaded_file.name}", key=uploaded_file.name):
    ...
    if all_data:
        ...
        st.download_button(
            ...
            file_name="all_invoice_data.xlsx",
